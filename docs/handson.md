...
```json
{"source":"qlm29h-gga","lat":35.68123600,"lng":139.76712500,"quality":4,"quality_label":"Fixed RTK","satellites":24,"hdop":0.60,"altitude_m":12.3,"utc_time":"03:04:05.000Z"}
```

Harvest DataはJSON内の数値をグラフにできます。`lat`と`lng`は地図表示に利用します。送信サイズの上限を考え、元のGGA全文は含めず、必要な項目だけにしています。
...
$GNGGA,030405.000,3540.874160,N,...
    ↓ カンマで分割・検証・座標変換
{"source":"qlm29h-gga","lat":35.68123600,...}
```
...
2. 最新データに`source`、`lat`、`lng`、`quality`などがあることを確認します。
...
- [ ] GGAのquality 4と5の違い
- [ ] 度分表記を十進数度へ変換する方法
- [ ] `lat`と`lng`をJSONへ入れる理由
- [ ] No Fixを送らない理由
- [ ] 通信断時に受信と送信を分離する理由
- [ ] at-least-once送信で重複が起こり得る理由
