...
### 3-4. 応用: NTRIP補正でRTK測位する

このハンズオンではNTRIP接続が準備済みですが、QLM29HがFixed RTKやFloat RTKになるまでには、次のデータの流れがあります。

```text
基準局 → NTRIP Caster → Raspberry Pi → RTCM → QLM29H
                                                ↓
                                      補正後のNMEA GGA
```

NTRIPは補正計算そのものではなく、RTCMなどのGNSSデータをインターネット経由で届ける仕組みです。QLM29Hが衛星の観測値とRTCM補正情報を使ってRTK測位を行い、結果をGGAの`quality=4`または`quality=5`として出力します。

VRS方式では、Rover付近の補正情報を生成するため、QLM29Hのおおよその位置を含むGGAをNTRIP Casterへ送る場合があります。

詳しい構成、用語、データの向き、補正が効かない場合の確認ポイントは[NTRIPで配信される補正情報とRTK測位](ntrip-rtk.md)で説明します。補正経路をコードで確認するためのPythonサンプルも紹介しますが、接続設定や認証情報の登録は本編では実施しません。
